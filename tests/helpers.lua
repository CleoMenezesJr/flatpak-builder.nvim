...
function Helpers.winsInTab(child, tab)
    tab = tab or "_G.FlatpakBuilder.state.activeTab"
...
Helpers.expect.buf_width = MiniTest.new_expectation(
    ...
    function(child, field, value)
        return Helpers.expect.equality(
            child.lua_get("vim.api.nvim_win_get_width(_G.FlatpakBuilder.state." .. field .. ")"),
            value
        )
    end,
    errorMessage
...
Helpers.expect.config = MiniTest.new_expectation(
    ...
    function(child, field, value)
        return Helpers.expect.global(child, "_G.FlatpakBuilder.config." .. field, value)
    end,
    errorMessage
)
...
Helpers.expect.config_type = MiniTest.new_expectation(
    ...
    function(child, field, value)
        return Helpers.expect.global(child, "type(_G.FlatpakBuilder.config." .. field .. ")", value)
    end,
    errorMessage
)

Helpers.expect.state = MiniTest.new_expectation("state matches", function(child, field, value)
    return Helpers.expect.global(child, "_G.FlatpakBuilder.state." .. field, value)
end, errorMessage)

Helpers.expect.state_type = MiniTest.new_expectation(
    "state type matches",
    function(child, field, value)
        return Helpers.expect.global(child, "type(_G.FlatpakBuilder.state." .. field .. ")", value)
    end,
    errorMessage
)
