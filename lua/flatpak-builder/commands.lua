...
function M.setup()
  ...
  if not utils.check_flatpak_builder() then
    utils.notify('flatpak-builder not found and fallback not available. Please install flatpak-builder or org.flatpak.Builder.', vim.log.levels.WARN)
  end
...
function M.build()
  ...
  if not manifest then
    utils.notify('No manifest file found. Use :FlatpakSelectManifest to select one.', vim.log.levels.ERROR)
    return
  end
  ...
  local cmd = string.format('%s %s %s %s --repo=%s',
                           flatpak_builder_cmd, options, build_dir, manifest, repo_dir)
...
function M.build_and_run()
  ...
  local cmd = string.format('%s %s %s %s --repo=%s --run %s',
                           flatpak_builder_cmd, options, build_dir, manifest, repo_dir, app_id)
